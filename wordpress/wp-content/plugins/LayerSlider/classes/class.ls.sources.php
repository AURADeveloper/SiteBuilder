<?php
require_once( dirname(__FILE__).'/../../glob-workaround.php' );
...
class LS_Sources {
	...
	public static function addSkins($path) {
		...
		if(file_exists($path.'/skin.css')) {
			...
			//$skinsPath = glob($path.'/*', GLOB_ONLYDIR); // todo appengine glob bug
			$skinsPath = get_folders($path);
		}
	...
	public static function addDemoSlider( $path ) {
		...
		if(file_exists($path.'/slider.zip')) {
			...
			//$slidersPath = glob($path.'/*', GLOB_ONLYDIR); // todo appengine glob bug
			$slidersPath = get_folders($path);
		}
